...
export function getRedirect(searchParams: URLSearchParams) {
    ...
    return redirect && Array.isArray(redirect) ? redirect[0] : redirect;
}

const humanLabel = (s: string) => humanize(toPascalCase(s));

export type SuccessContext<T> = { response?: T };
export type SuccessEventHandler<T> = (
    ctx: SuccessContext<T>
) => Promise<any> | void;
...
export const Form: FC<FormProps> = (props) => {
    const { className, method, onSubmit, onSuccess, children, ...remaining } =
        props;
    ...
    return (
        <ApiContext.Provider value={{ loading, error }}>
            <form
                method={method ?? "POST"}
                className={className}
...
export const ErrorSummary: FC<ErrorSummaryProps> = ({
    ...
    const errorSummary = ctx.responseStatus
        ? errorResponseExcept.call(ctx, except ?? [])
        : null;
    ...
    return (
        <div
            className={classNames(
                "relative flex items-center border p-3.5 rounded before:inline-block before:absolute before:top-1/2 ltr:before:right-0 rtl:before:left-0 rtl:before:rotate-180 before:-mt-2 before:border-b-8 before:border-t-transparent before:border-b-transparent before:border-r-inherit text-danger bg-danger-light border-danger dark:bg-danger-dark-light",
                className
            )}
        >
            <span className="ltr:pr-2 rtl:pl-2">{errorSummary}</span>
        </div>
    );
};
...
export const TextInput: FC<TextInputProps> = ({
    ...
    const cssClass = (validCls?: string, invalidCls?: string) => [
        !hasErrorField ? validCls : invalidCls,
        className,
    ];
        ...
        <div>
            ...
            <div className="mt-1 relative rounded-md shadow-sm">
                <input
                    ...
                    className={classNames([
                        input.base,
                        ...cssClass(input.valid, input.invalid),
                    ])}
                    id={id}
                    name={id}
...
export const SelectInput: FC<SelectInputProps> = ({
    ...
    return (
        <>
            ...
            <select
                className={classNames([
                    "mt-1 block w-full pl-3 pr-10 py-2 text-base focus:outline-none border-gray-300 dark:border-gray-600 sm:text-sm rounded-md",
                    ...cssClass(
    ...
    return (
        <>
            <input
                ...
                onChange={onChange}
            />
            <label htmlFor="confirmDelete" className="mx-2 select-none">
                confirm
            </label>
...
export const Loading: FC<LoadingProps> = ({ className, icon, text }) => {
    ...
    let cls = ["flex", "items-center", className]; // Add "items-center" to vertically align

    return (
        <div className="fixed inset-0 flex items-center justify-center">
            <div className={cls.join(" ")} title="Vui lòng chờ một chút ...">
                {showIcon ? (
                    <svg
                        version="1.1"
                        xmlns="http://www.w3.org/2000/svg"
                ...
                <span className="ml-2 text-gray-400">{showText}</span>
            </div>
        </div>
    );
};
