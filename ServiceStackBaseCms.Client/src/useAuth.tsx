...
export function ValidateAuth<TOriginalProps extends {}>(
    ...
    const compWithProps: React.FC<TOriginalProps & AuthenticatedContext> = (
        props
    ) => {
...
export function useAuth({}: Props = {}): OptionalAuthContext {
    const {
        ...
        mutate: revalidate,
        error,
    } = useSWR(KEY, (_) => client.post(new Authenticate()));
    ...
    const hasRole = (role: string) =>
        (auth?.roles || []).indexOf(role) >= 0 || isAdmin();
    const hasPermission = (permission: string) =>
        (auth?.permissions || []).indexOf(permission) >= 0 || isAdmin();
